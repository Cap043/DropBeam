...
export default function ReceivePage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  return (
    <div className="p-10 flex flex-col gap-4 max-w-md text-black">
      <h1 className="text-xl font-bold text-white">Receiver</h1>
      <p className="text-white">Joined Room: <strong className="text-blue-400">{resolvedParams.roomId}</strong></p>
      ...
      {incomingFile && progress === 0 && !isDone && (
        <div className="border p-4 bg-yellow-50 rounded flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={handleAccept} className="p-2 bg-green-600 text-white rounded flex-1 font-bold">
              Accept & Save
            </button>
            <button onClick={handleCancel} className="p-2 bg-red-600 text-white rounded font-bold">
              Reject
            </button>
          </div>
        </div>
      ...
      {isDone && (
        <div className="border p-4 bg-green-50 rounded flex flex-col gap-3">
          ...
          <button onClick={resetReceiver} className="p-2 bg-blue-600 text-white rounded font-bold">
            Ready for Next File
          </button>
        </div>
      )}
